...
public class Gl3Sample implements GLEventListener {
    ...
    void newFloatVertexAndColorBuffers(GL3 gl, int vaoId, float[] verticesArray, float[] colorArray, int verticeLoc, int colorLoc){
        ...
        gl.glBufferData(GL.GL_ARRAY_BUFFER, verticesArray.length * Float.SIZE / 8,
                Buffers.newDirectFloatBuffer(verticesArray), GL.GL_DYNAMIC_DRAW);
        gl.glEnableVertexAttribArray(vertexLoc);
        gl.glVertexAttribPointer(vertexLoc, 4, GL.GL_FLOAT, false, 0, 0);
    ...
    void bindBuffer(GL3 gl, int bufferId, float[] dataArray, int dataLoc){
        ...
        gl.glBufferData(GL.GL_ARRAY_BUFFER, dataArray.length * Float.SIZE / 8,
                Buffers.newDirectFloatBuffer(dataArray), GL.GL_DYNAMIC_DRAW);
    ...
    protected void renderScene(GL3 gl) {
        ...
        GLUniformData data = new GLUniformData("projMatrix", 4, 4, Buffers.newDirectFloatBuffer(this.projMatrix));
        data.setLocation(this.projMatrixLoc);
        gl.glUniform(data);
    ...
    int newProgram(GL3 gl) {
        ...
        int v = this.newShaderFromCurrentClass(gl, "C:\\Users\\mykhailo\\HYT\\HYTAu\\src\\main\\resources\\v.glsl", ShaderType.VertexShader);
        int f = this.newShaderFromCurrentClass(gl, "C:\\Users\\mykhailo\\HYT\\HYTAu\\src\\main\\resources\\f.glsl", ShaderType.FragmentShader);
